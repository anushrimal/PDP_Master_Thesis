...
bool RpiAuth::validate(vector<int> b, vector<string> h, string& topHash)
{
	...
	*/
	...
	for(int i = 0; i < b.size(); i++) {
		...
		for(int j = this->levels - 1, k = 1; j >= 0 && k <= this->levels; j--, k++) {
			low_bound = (cur_rpi / pow(this->order, k))*(this->order);
			up_bound = low_bound + this->order;
			int start_idx = 0, end_idx = 0;
			for(int l = 0; l < rpi.size(); l++) {
				if(rpi[l] >= low_bound && rpi[l] <= up_bound) {
					//TODO
					continue;	
				}
			}
			// = find(rpi.begin(), rpi.end(), low_bound) - rpi.begin();
			//int end_idx = find(rpi.begin(), rpi.end(), up_bound) - rpi.begin();
			for(int idx = start_idx; idx <= end_idx; idx++) {
				if(j == this->levels - 1) {
					if(first) {
						if(idx == b_idx[i]) //Use hash from input vector
							str_to_uchar(h[i], r_hash);
						else
							str_to_uchar(h[idx], r_hash);
						first = false;
						continue;
					} else {
						if(idx == b_idx[i])
							...
							str_to_uchar(h[idx], c_hash); 
					}
					hash_add(r_hash, c_hash, r_hash);
				} else {
					if(rpi[idx] == cur_rpi/(pow(this->order, k))) //parent of current node {
						continue;	
				}		 
			}		
		}
